...
function useGetData(studyType, sponsor = '*', filterOnDrug = false) {
  ...
  async function getDataAxios() {
    ...
    observationSnaps.forEach((observationSnap) => {
      const observationYear = parseInt(observationSnap.substring(0, 4), 10);
      const queryHasResults = getFetchOptions({
        key: 'studiesDynamiqueOuverture',
        parameters: [studyType, observationYear - 10, observationYear - 1, observationSnap],
        objectType: ['clinicalTrials'],
      });
      if (filterOnDrug) {
    ...
    observationSnaps.forEach((observationSnap, index) => {
      ...
      data.push({
        ...
        yearMax: observationYear - 1,
        yearMin: observationYear - 10,
      });
      data.push({
